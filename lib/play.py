...
videoFile = xbmc.translatePath(xbmcaddon.Addon().getAddonInfo('profile')+'/video.m3u8').decode('utf-8')
bookFile = xbmc.translatePath(xbmcaddon.Addon().getAddonInfo('profile')+'/video.chapters.xml').decode('utf-8')
subFile = xbmc.translatePath(xbmcaddon.Addon().getAddonInfo('profile')+'/video.srt').decode('utf-8')
...
def _saveBookmarks(content):
	data = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
	data += '\n<chapters>'
	if '<cuepoints>' in content:
		cuepoints = re.compile('<cuepoints>(.+?)</cuepoints>', re.DOTALL).findall(content)[0]
		match = re.compile('<value key="(.+?)">(.+?)</value>', re.DOTALL).findall(cuepoints)
		for key, value in match:
			data += '\n<chapter mark="'+key+'" title="'+value+'" />'
	data += '\n</chapters>'
	utils.f_write(bookFile,data)